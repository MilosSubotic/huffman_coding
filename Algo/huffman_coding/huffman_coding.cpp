...
namespace huffman_coding {
	...
	void huffman_encode(
		...
		auto pack = [&](uint16_t bits, unsigned len) {
			...
			acc_len += len;
			if(acc_len >= 8){
				out_enc_data.push_back(acc & 0xff);
				acc >>= 8;
				acc_len -= 8;
			}
		};

		for(int block = 0; block < in_data.size()/max_block_len; block++){
			cout << "block: " << block << endl << endl;

			vector<sym_t> in_data_block(max_block_len);

			for(int d = 0; d < 16; d++){
				int index = block*max_block_len + d;
			...
			for(int d = 0; d < 16; d++){
				cout << setw(2)
					<< in_data_block[block*max_block_len + d] << endl;
			}
			cout << endl << endl;
			...
			vector<freq_t> histogram(freq_num, 0);

			for(int d = 0; d < freq_num; d++){
				histogram[in_data_block[d]]++;
			}

			cout << "histogram:" << endl;
			for(int sym = 0; sym < freq_num; sym++){
	...
	void huffman_decode(
		...
		for(int d = 0; d < out_data.size(); d++){
			cout << setw(2) << d << ": " << setw(2) << out_data[d] << endl;
		}
		cout << endl << endl;
